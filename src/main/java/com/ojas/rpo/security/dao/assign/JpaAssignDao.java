...
public class JpaAssignDao extends JpaDao<Assign, Long> implements AssignDao {
	...
	public Response getReqByRecIdandUserId(Long userId, String status, Integer pageNo, Integer pageSize,
		...
		Query query = null;
		// Query q = getEntityManager().createQuery(" select ass.bdmReq from Assign ass
		...
		StringBuilder sql = new StringBuilder(
				"SELECT br.id as reqId,c.id as clientId, a.id as assinedId,br.nameOfRequirement,c.clientName,ac.contact_Name,ac.mobile,ac.email,br.status,br.requirementStartdate,br.requirementEndDate,br.noticePeriod FROM `assign` a "
		...
		try {
			...
			if (results.isEmpty()) {
				...
				for (Object[] data : results) {
					...
					respData.setMobile(data[6].toString());
					respData.setEmail(data[7].toString());
					respData.setStatus(data[8].toString());
					respData.setStartDate((Date) data[9]);
					respData.setEndDate((Date) data[10]);
					respData.setNoticePeriod(data[11].toString());
					result.add(respData);
				}
		...
		try {
			...
			if (role.equalsIgnoreCase("BDM") || role.equalsIgnoreCase("bdmlead")) {
				...
				if (null != searchField && null != searchText && !searchField.equalsIgnoreCase("undefined")
					...
					sqlBuilder = sqlBuilder.append(" WHERE (c.primaryContact_id=? OR c.secondaryContact_id=?) AND "
				...
				countQuery = countQuery + " WHERE (c.primaryContact_id=? OR c.secondaryContact_id=?) ";
				...
				if (null != searchField && null != searchText && !searchField.equalsIgnoreCase("undefined")
						&& !searchText.equalsIgnoreCase("undefined")) {
					sqlBuilder = sqlBuilder.append("WHERE (c.accountManger_id = ?) AND " + searchField + " LIKE '%"
							+ searchText + "%' ORDER BY " + sortingField + " " + sortingOrder + " LIMIT " + startingRow
							+ ", " + pageSize);
				} else {
					sqlBuilder = sqlBuilder.append("WHERE (c.accountManger_id = ?) ORDER BY " + sortingField + " "
							+ sortingOrder + " LIMIT " + startingRow + ", " + pageSize);
				}

				q = getEntityManager().createNativeQuery(sql);

				countQuery = countQuery + " WHERE (c.accountManger_id = ?) ";
				Query count = this.getEntityManager().createNativeQuery(countQuery);
				count.setParameter(1, userId);
				totalRecords = Integer.valueOf(count.getSingleResult().toString());
			}
	...
	public Response searchAssignedRequirementsByBdmId(String role, Long id, String searchInput, String searchField,
		...
		// String countQuery = "select COUNT(*) FROM assign asn INNER JOIN client c ON
		// asn.client_id = c.id INNER JOIN bdmreq r ON asn.bdmReq_id=r.id INNER JOIN
