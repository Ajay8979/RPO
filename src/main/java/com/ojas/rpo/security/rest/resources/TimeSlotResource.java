...
public class TimeSlotResource {
	...
	public @ResponseBody Response create(TimeSlot timeslot) {

		if (timeslot == null) {
			return new Response(ExceptionMessage.DataIsNotSaved);
		} else {
			timeslot = this.timeSlotDao.save(timeslot);
			Candidate candidate=timeslot.getCandidate();
			candidate.setCandidateStatus("Interview Schedule Rejected By Candidate");
			candidateDao.save(candidate);
			Long candidateId=candidate.getId();
			InterviewDetails interviewDetails=interviewDetailsDao.getInterviewDetailsByCandidateId(candidateId);
	...
	public @ResponseBody Response candidateInterviewScheduleStatus(@PathParam("candidateId") Long cid,
		...
		if (candidate.getCandidateInterviewScheduleStatus().equalsIgnoreCase("accepted")) {
			candidate.setCandidateStatus("Interview Scheduled");

			this.candidateDao.save(candidate);
		} else if (candidate.getCandidateInterviewScheduleStatus().equalsIgnoreCase("rejected")) {
