...
public class UserRegistrationResource {
	...
	public @ResponseBody Response create(User user, @Context HttpServletRequest request) {
		User adminUser = new User(user.getUsername(), this.passwordEncoder.encode(user.getPassword()),
		...
		emailEntity.setMessagesubject(" Registred Successfully...");// add subject
		emailEntity.setTo(user.getEmail());// add to address

		SimpleDateFormat formatter = new SimpleDateFormat("dd-MM-yyyy");

		String mes = "<i>Welcome</i><B>" + "  " + user.getFirstName() + ",</B><br><br><br>" +

				"You can use the following credentials to access the <B>" + "RPO" + "</B> portal<br>" + "<a href ="
				+ request.getContextPath() + ">Click Here to Login</a> + <br><br>" +
				"Your username is :" + user.getName() + "<br>" + "Your password is :" + user.getPassword() + "<br><br>"
		...
		// send mail
		OutlookMailSender mailSender = new OutlookMailSender();
		...
		if (user.getRole().equals("bdm")) {
			adminUser.addRole(Role.BDM);
		}
		if (user.getRole().equals("recruiter")) {
			adminUser.addRole(Role.RECRUITER);
		}

		if (user.getRole().equals("AM")) {
			adminUser.addRole(Role.AM);
		}
		if (user.getRole().equals("Lead")) {
			adminUser.addRole(Role.TEAMLEAD);
		}

		// return this.userDao.save(adminUser);

		if (this.userDao.save(adminUser) == null) {
			return new Response(ExceptionMessage.DataIsNotSaved);

		} else {
			return new Response(ExceptionMessage.StatusSuccess, adminUser);
	...
	public Response usersList(@PathParam("pageNo") Integer pageNo, @PathParam("pageSize") Integer pageSize,
		...
		Response res=this.userDao.findAllUsers(pageNo, pageSize, sortingOrder, sortingField, searchField, searchInput);
