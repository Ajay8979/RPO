...
public class BdmRequirement {
	...
	public @ResponseBody Response getdata(@PathParam("id") Long id, @PathParam("status") String status,
		...
		this.logger.info("read(id)");
		Response response = this.bdmReqDao.findreqByClientId(id, status,pageNo,pageSize,sortOrder,sortField,searchText,searchField);
	...
	public @ResponseBody Response getBdmRequirementsByRole(@PathParam("id") Long id, @PathParam("role") String role,
		...
		return this.bdmReqDao.getBdmReqirementsByRole(id, role, pageNo, pageSize, sortingOrder, sortingField, searchText, searchField);
