app.controller("TypeOfProcessController",function($scope, $location, $routeParams, $filter, $rootScope, $window,
						TypeOfProcessService, invoiceServices, invoiceService1,$http,$q,
						selectedDetailsService,$timeout) {
			...
			debugger;				
							 var param1 = $routeParams.id;
							  	  $http.get('rest/candidateReqMapping/getCandiateMapCandedateId/'+param1).then(function(response){
								  		 ...
								  		 $scope.candidateiname = $scope.reqCanData[0].candidate.firstName+' '+ $scope.reqCanData[0].candidate.lastName;
							  	  ...
							  	  //remaining details for interview progress after saving
							  	    $http.get('rest/interviewDetails/getInterviewDetailsById/'+param1).then(function(response){
							  	    	
							  	    	$scope.interviewData=response.data.result;
							  	    	$scope.candidateiname = response.data.result.candidate.firstName+' '+ response.data.result.candidate.lastName;
							  	        ...
							  	        $window.localStorage.setItem('requirementID',response.data.result.requirement.id);

							  	    	console.log("rest/interviewDetails/getInterviewDetailsById", response.data);
							  	        console.log("candidateID,requirementID",response.data.result.candidate.id,response.data.result.requirement.id)
							  			
							  	  });
							  		
									//getting the rejected details
							
							  	    var candidate_id = $window.localStorage.getItem('candidateID');
							  	    var requirement_id = $window.localStorage.getItem('requirementID');
							  	  $http.get('rest/timeSlots/getCandidateTimeSlot/'+candidate_id+'/'+requirement_id)
								  .then(function(response) {
									console.log("rest/timeSlots/getCandidateTimeSlot",response.data);
...
debugger;
								$scope.canStatus=sessionStorage.getItem('canStatus');
								debugger;
							  	$http.get('rest/addCandidate/candidateStausList/'+$scope.canStatus).then(function(response){
								   	        $scope.processData=response.data.result;
								   	        console.log("rest/addCandidate/candidateStausList",response.data.result);								   	      
								  		});	
					...
					//Upload All Files Start....
					 ...
					 $rootScope.fileName;
	                  ...
	                  $scope.upload12ThMM=function(){
	                	 if($scope._12thMarksMemo == undefined){
	                		...
	                		return;
	                		 ...
	                		 if($scope._12thMarksMemo !=undefined){
	 	                		var ext = $scope._12thMarksMemo.name.split('.').pop();
	 	               		 console.log(ext);
	 	               		 if(ext=='pdf'){
	                  ...
	                  $scope.uploadHMM=function(){
	                	...
	                	if($scope._highestMarkmemo!=undefined){
	                		var ext = $scope._highestMarkmemo.name.split('.').pop();
	                  		 console.log(ext);
	                  		 if(ext=='pdf'){
	                  ...
	                  $scope.uploadLatestSalaryslips=function(){
	                	...
	                	if($scope._latestSalaryslips!=undefined){
	                		var ext = $scope._latestSalaryslips.name.split('.').pop();
	                  		 console.log(ext);
	                  		 if(ext=='pdf'){
	                  ...
	                  $scope.uploadBankStatements=function(){
	                	...
	                	if($scope._bankStatements!=undefined){
	                		var ext = $scope._bankStatements.name.split('.').pop();
	                  		 console.log(ext);
	                  		 if(ext=='pdf'){
	                  ...
	                  $scope.uploadOfferedLetter=function(){
	                	...
	                	if($scope._offeredLetter!=undefined){
	                		var ext = $scope._offeredLetter.name.split('.').pop();
	                  		 console.log(ext);
	                  		 if(ext=='pdf' ){
	                  ...
	                  $scope.uploadRelieving=function(){
	                	...
	                	if($scope._RelievingLetter!=undefined){
	                		var ext = $scope._RelievingLetter.name.split('.').pop();
	                  		 console.log(ext);
	                  		 if(ext=='pdf' ){
	                  ...
	                  $scope.uploadPassportCopy=function(){
	                	...
	                	if($scope._passportCopy!=undefined){
	                		var ext = $scope._passportCopy.name.split('.').pop();
	                  		 console.log(ext);
	                  		 if(ext=='pdf'){
	                  ...
	                  $scope.uploadForm16=function(){
	                	...
	                	if($scope._form16Certificate!=undefined){
	                		var ext = $scope._form16Certificate.name.split('.').pop();
	                  		 console.log(ext);
	                  		 if(ext=='pdf'){
	                  ...
	                  $scope.uploadasZip =function(){
		            	   debugger;
		                	...
		                	if($scope.uploadZip!=undefined){
		                		 var ext = $scope.uploadZip.name.split('.').pop();
		                		 console.log(ext);
		                		 if(ext=='zip'){
	                  ...
	                  $scope.allUpload=function(){
						  ...
						  $http.post("rest/addCandidate/uploadZip/"+id,formData,hd).success(function(response){ 
...
//						  }
	               ...
	               $scope.imageUpload=function(){
						  ...
						  $http.post("rest/addCandidate/upload/"+fileName+"/"+id,formData,hd).success(function(response){ 
							 console.log(response);
...
//						  }
	               ...
	               $scope.save = function(){
	            	   ...
	            	   var deferred = $q.defer();
	            	   $http.post("rest/addCandidate/updatingstatusbylead/"+id+"/"+ status).success(function(response){
	            		  console.log(response); 
	            		  if (respose.status="StatusSuccess"){
...
//	            	   var status = "Offer Release";
	            	   ...
	            	   var deferred = $q.defer();
	            	   $http.post("rest/addCandidate/updatingOfferStatus/"+id+"/"+offerStatus).success(function(response){
	            		  console.log(response); 
	            		  if (respose.status="StatusSuccess"){
...
//	            	   var status = "Offer Release";	
	            	 /*  var onBoardStatus= $scope.interviewstatus;
	            	   ...
	            	   var deferred = $q.defer();
	            	   $http.post("rest/addCandidate/updatingOnBoardStatus",onBoardFields).success(function(response){
	            		  console.log(response.status); 
	            		  if (respose.status="StatusSuccess"){
	            			  alert("Raise Invoice Status Updated")
	            		  }
	            	   });
...
//	            	   var status = "Offer Release";
	            	  ...
	            	  var ctcStatus={
	            	   ...
	            	   var deferred = $q.defer();
	            	   $http.post("rest/addCandidate/getCtcDetails",ctcStatus).success(function(response){
	            		debugger;
	            		  console.log(response); 
	            		  ...
	            		  console.log("CTC status"+$scope.ctcdeduction);
	            		  console.log("employeeEarning status"+$scope.employeeEarning);
...
//	            	   var status = "Offer Release";
	            	   ...
	            	   $http.post("rest/addCandidate/updatingOfferStatus/"+id+"/"+offerStatus).success(function(response){
	            		  console.log(response); 
	            		  if (respose.status="StatusSuccess"){
	            			  alert("On Borad Status updated Successfully")
	            		  }
	            	   });
...
/*                  app.directive('fileModel', ['$parse', function ($parse) {
				...
				debugger;	                                  
					         $http.post("rest/addCandidate/post_12thMarkmemo/"+id,file) .then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
								 $scope.uploadS2=true;
				  ...
				  $scope.save_highestMarkmemo=function(file)
				  {
					  ...
					  if(file==undefined)
					  {
						  ...
						  return;
					         ...
					         $http.post("rest/addCandidate/post_highestMarkmemo/"+id,file) .then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
								 $scope.uploadS3=true;
				   ...
				   $scope.save_latestSalaryslips=function(file)
				  {
					  ...
					  if(file==undefined)
					  {
						  ...
						  return;
					         ...
					         $http.post("rest/addCandidate/post_latestSalaryslips/"+id,file) .then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
								 $scope.uploadS4=true;
				   ...
				   $scope.save__bankStatements=function(file)
				  {
					  ...
					  if(file==undefined)
					  {
						  ...
						  return;
					         ...
					         $http.post("rest/addCandidate/post_bankStatements/"+id,file) .then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
								 $scope.uploadS5=true;
				   ...
				   $scope.save_offeredLetter=function(file)
				  {
					  ...
					  if(file==undefined)
					  {
						  ...
						  return;
					         ...
					         $http.post("rest/addCandidate/post_offeredLetter/"+id,file) .then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
								 $scope.uploadS6=true;
				   ...
				   $scope.save_RelievingLetter=function(file)
				  {
					  ...
					  if(file==undefined)
					  {
						  ...
						  return;
					         ...
					         $http.post("rest/addCandidate/post_RelievingLetter/"+id,file) .then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
								 $scope.uploadS7=true;
				   ...
				   $scope.save_passportCopy=function(file)
				  {
					  ...
					  if(file==undefined)
					  {
						  ...
						  return;
					         ...
					         $http.post("rest/addCandidate/post_passportCopy/"+id,file) .then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
								 $scope.uploadS8=true;
				    ...
				    $scope.save_form16Certificate=function(file)
				  {
					  ...
					  if(file==undefined)
					  {
						  ...
						  return;
					         ...
					         $http.post("rest/addCandidate/post_form16Certificate/"+id,file) .then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
								 $scope.uploadS9=true;
				    ...
				    $scope.uploadZip=function(file)
					  {
						  ...
						  if(file==undefined)
						  {
							  ...
							  return;
						         ...
						         $http.post("rest/addCandidate/uploadZip/"+id,file) .then(function(response) {
	                              console.log(response);
								  if(response.data.status=="StatusSuccess"){
				  ...
				  $scope.finalSubmit=function(){
					  if($scope.uploadS1 && $scope.uploadS2  && $scope.uploadS3 && $scope.uploadS4 && $scope.uploadS5 && $scope.uploadS6 && $scope.uploadS7 && $scope.uploadS8 && $scope.uploadS9)
					  {	  
				 var id = $window.localStorage.getItem("candidateid");
				 
				 $http.post('rest/addCandidate/updatingstatus/test/'+id).then(function(response) {
                              console.log(response);
							  if(response.data.status=="StatusSuccess"){
				   ...
				   $scope.saveAccept=function()
				  {
					...
					$http.post("rest/addCandidate/accepteddata/"+id,values) .then(function(response) {
                          console.log(response);
						 if(response.data.status=="StatusSuccess"){
						 $scope.uploadS1=true;
						 $scope.uploadE1=false;
...
//									$scope.companyname = $scope.candidateDetails.client.clientName;
									...
									var date=new Date($scope.candidateDetails.doj).getDate();
									var month=new Date($scope.candidateDetails.doj).getMonth();
									var year=new Date($scope.candidateDetails.doj).getYear();
									$scope.doj1=date+"/"+month+"/"+year;
...
//									$scope.bid=$scope.candidateDetails.bdmReq.id;
									...
									$scope.amid=$window.localStorage.getItem("usid");
						...
						$rootScope.processDetails=[];
							...
							$scope.process1=function(){
								//$scope.status=sessionStorage.getItem('canStatus'); 
							var rr = $window.localStorage.getItem("role");
							var process = $rootScope.process
							TypeOfProcessService.getprocessData(rr,process).then(function(response){
					...
					$scope.interviewtype = function() {
						...
						promise.then(function(response) {
							...
							console.log("[get interview type from 'TypeOfProcessService'] interviewDetails ", $scope.interviewDetails);
						});
					...
					$scope.hiringChange = function() {
						...
						if ($scope.fbk.recRound == "Hire") {
						 $http.get('rest/addstatus/findstatusforc2h').then(function(response){
					  		  $scope.status = response.data.result;
					   	        console.log("rest/addstatus/findstatusforc2h",response.data.result);
					  	  });	}
						else{
						 $http.get('rest/addstatus/findstatus').then(function(response){
					  		  $scope.status = response.data.result;
					   	        console.log("rest/addstatus/findstatus",response.data.result);
					  	  });	
						 }
					}
					$scope.interviewchange = function() {
                  debugger;        
//						 alert($scope.processtype.status );
			...
			//$scope.interviewstatus();
					...
					$scope.submitInterviewProcess=function(){
						debugger
					var obj={
						candidate:{id:$rootScope.candidate_id},
						requirement:{id:$scope.reqId},
						spocName:$scope.spocName,
						interviewDate:$scope.interviewDate,
						interviewTime:$scope.interviewTime,
						nameOfRound:$scope.round,
						interviewType:{id:$scope.typeofinterview},
						interviewLocation:$scope.interviewlocation,
						address:$scope.interviewAddress,
					    }
					
						$http.post('rest/interviewDetails',obj).then(function(response) {
		            console.log("[post] rest/interviewDetails", obj);
								 if(response.data.status=="StatusSuccess"){
...
//					// update interview status
					...
					var candidate_Id = $window.localStorage.getItem('candidateid');
					$http.post('rest/interviewDetails/updateInterviewStatusDetails/'+candidate_Id).then(function(response) {
						console.log("update interview status",response);
						 $location.path("/CandidateList1")
					});
			...
			//==================================================================================		
				//submitting the rejected details	
					$scope.submitRejected=function(){
						var rej={
								slot1Time:$scope.slot1_time,
								slot2Time:$scope.slot2_time,
								slot3Time:$scope.slot3_time,
								slot1Date:$scope.slot1_date,
								slot2Date:$scope.slot2_date,
								slot3Date:$scope.slot3_date,
								reason:$scope.reason,
								candidate:{id:$rootScope.candidate_id},
								requirement:{id:$scope.reqId},						
						    }
							$http.post('rest/timeSlots',rej).then(function(response) {
			            console.log("submitRejected", response);
			            $location.path('/CandidateList1');
//									// if(response.data.status=="StatusSuccess"){
//									// alert("Submitted Successfully")		
//									
		...
		//	==========================================================================================
					...
					$scope.navigationToOffeLetter=function(){
						...
						$scope.getInterviewShedule= function() {
							...
							var nid =  angular.fromJson($window.localStorage
								.getItem('candidateid'));
					   console.log(";;;;;;;;;;;;;;;"+nid);
						var promise = TypeOfProcessService.getInterviewDetails(nid);
						promise.then(function(response) {
							$scope.Interviewdetails = response.data.result;
							...
							$scope.hj=new Date($scope.Interviewdetails.interviewdate);
							$scope.idate1=($scope.hj.getDay()+"/"+$scope.hj.getMonth()+"/"+$scope.hj.getFullYear());
							$scope.mobile1=$scope.Interviewdetails.mobilenumber;
							$scope.pincode1=$scope.Interviewdetails.pincode;
							console.log($scope.Interviewdetails.pincode1);
							$scope.State=$scope.Interviewdetails.state;
							console.log($scope.Interviewdetails.time);
				...
				$scope.getInterviewShedule();
					...
					$scope.saveInterviewFeedbackforRejeted = function() {
						...
						console.log("feedbackrejectedddata",feedbackrejectedddata);

						TypeOfProcessService.sendFeedbackData(feedbackrejectedddata);
						//$scope.process1();
							$location.path("/CandidateList1");
							//$scope.process1();
					       // $window.location.reload();
			...
			$scope.getInterviewFeedback= function() {
				debugger;
							var gid =  angular.fromJson($window.localStorage
								.getItem('candidateid'));
						var promise = TypeOfProcessService.getFeedbackDetails(gid);
						promise.then(function(response) {
...
//						}
					...
					$scope.saveSbmitted = function() {
					 ...
					 $http.post('rest/addCandidate/offereletter/'+id,values).then(function(response) {
                     console.log(response);
						 if(response.data.status=="StatusSuccess"){
					...
					$scope.savesubcust = function() {
						...
						console.log(submiteedlistvalues);
						TypeOfProcessService.sendcustsubData(
								submiteedlistvalues, id);
							//	$scope.process1();
								$location.path("/CandidateList1");
						     //   $scope.process1();
						      //  $window.location.reload();
					...
					$scope.savecustreject = function() {
						...
						TypeOfProcessService.sendCustrejectedData(
								custrejectvalues, id);
								$scope.process1();
								$location.path("/CandidateList1");
						       //$scope.process1();
