...
public class ProcessResource {
	...
	public @ResponseBody  Response create(Processtype process) {
		System.out.println("==========inside post method====RPO");
		this.logger.info("create(): " + process);
	
		Processtype am=null;
		this.logger.info("create(): " + process);
		if (process.getTypeofprocess().equalsIgnoreCase("Customer Shortlisted")) {
			process.setTypeofprocess("Process for Interview");
			am= this.processdao.save(process);
		}
		if(am!=null)
		{
			 this.processdao.updateCandiate(process.getCandidateid(), process.getTypeofprocess());
	...
	public @ResponseBody  Response getdata() throws IOException {
		...
		public @ResponseBody  Response updatestatus(@PathParam("id") Long id) {
			...
			Processtype processtype = this.processdao.findById(id);
				...
				if(processtype!=null)
				{
				 this.processdao.updateCandiate(id, processtype.getTypeofprocess());
