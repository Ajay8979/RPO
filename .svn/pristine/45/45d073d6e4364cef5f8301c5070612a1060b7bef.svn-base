...
public class AddContactResources {
	...
	@GET
	@Produces(MediaType.APPLICATION_JSON)
	@Path("/findActiveContactByClientId/{id}")
	public @ResponseBody Response findActiveContactByClientId(@PathParam("id") Long id) {
		this.logger.info("read(id)");

		List<AddContact> addContactlist = this.addClientContactDao.findActiveContactByClientId(id);
	...
	public @ResponseBody Response findContactByBdmId(@PathParam("id") Long id, @PathParam("role") String role,
		...
		return this.addClientContactDao.findContactByBdmId(id, role, sortField, sortOrder, searchField, searchText, pageNo, pageSize);
	}
	 ...
	 * findContactByBdmId(@PathParam("id") Long id, @PathParam("role") String role)
	 * { this.logger.info("read(id)");
	 * 
	 ...
	 * Response(ExceptionMessage.StatusSuccess, addContactlist); } else { return new
