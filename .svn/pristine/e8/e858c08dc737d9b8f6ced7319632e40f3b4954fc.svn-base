app.service('assignListwrkSer', function($http, $q) {
	this.assignfunservice = function(pageIndex,pageSizeSelected,sortingOrder,sortingField,searchtext,searchcategory) {
       ...
       var userRole=sessionStorage.getItem('userRole');
		...
		$http.get('rest/allocaterequirment/getAssinedRequirementsByBdmId/'+UserId+'/'+userRole+'/'+pageIndex+'/'+pageSizeSelected+'?sortingOrder='+sortingOrder+'&sortingField='+sortingField+'&searchText='+searchtext+'&searchField='+searchcategory).then(
				function(data) {
